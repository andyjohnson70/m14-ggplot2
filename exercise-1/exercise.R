...
ggplot(diamonds.sample, mapping = aes(x = 'carat', y = 'price'))
...
ggplot(diamonds.sample) + geom_point(mapping = aes(x = cut, y = carat, size = price))


# Try coloring the above plot based on the diamond's price!
ggplot(diamonds.sample) + geom_point(mapping = aes(x = cut, y = carat, size = price, color = price))
...
ggplot(diamonds.sample) + geom_histogram(mapping = aes(x = price, fill = clarity))
...
ggplot(data = diamonds.sample) + geom_point(mapping = aes(x = carat, y = price, color=cut), alpha=0.1) + geom_smooth(mapping = aes(x = carat, y = price, color=cut), se=FALSE)
